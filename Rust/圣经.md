...
#### `for`循环，遍历集合内容
   ...
   9. 使用迭代器而非索引访问集合，更简洁也更安全，同时避免`运行时边界检查`，性能更高
      ...
      //获取索引和对应元素
      for (i,e) in container.iter().enumerate(){
...
#### 特征`trait`
   ...
   8. 方法中使用特征约束
         ...
         ```rust
         impl <T:Display + PartiaOrd> Pair<T>{}
         ```
...
1. 签名，`fn useless<'a>(first:&'a i32, second:&'a i32) {}`
...
2. 生命周期约束语法`<'a :'b>`
   ...
   ```rust
   impl<'a:'b, 'b>Import<'a>{
      fn announce(&'a self, anno: &'b str) -> &'b str{}
   }
